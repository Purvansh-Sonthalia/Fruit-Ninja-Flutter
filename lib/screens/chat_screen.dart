...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      final pickedFile = await picker.pickImage(
          source: source, imageQuality: 70); // Adjust quality as needed
  ...
  Widget _buildMessagesList(bool isLoading, bool hasError, String errorMessage,
    ...
    if (hasError && messages.isEmpty) {
      // Show error only if list is empty, otherwise show list + error?
      return Center(
        child: Padding(
          ...
          messageItemWidget = GestureDetector(
            ...
            onLongPress: () {
              ...
              _showDeleteConfirmation(context,
                  message); // Or potentially call a method on messageItemWidget if needed
            },
      ...
      separatorBuilder: (context, index) {
        ...
        if (index < messages.length - 1) {
          final currentMessage =
              messages[index]; // Newer message (due to reverse)
          ...
          if (timeDifference.abs() > const Duration(hours: 1)) {
            return const SizedBox(
                height: 30.0); // Adjusted gap slightly for balance
          }
        }
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: () => _pickImage(
                    ImageSource.gallery), // Or show options for gallery/camera
...
class _ChatMessageListItem extends StatefulWidget {
  ...
  final Function(BuildContext, Message)
      onDeleteLongPress; // Callback for delete
...
class _ChatMessageListItemState extends State<_ChatMessageListItem>
      ...
      builder: (context, snapshot) {
              ...
              if (parentHasImage && parentImageBytes != null)
                Padding(
                  padding: const EdgeInsets.only(
                      top: 1.0), // Add slight padding if text follows
                  child: Row(
                    mainAxisSize:
              ...
              if (parentMessage.messageText != null &&
                ...
                Padding(
                  padding: EdgeInsets.only(
                      top: parentHasImage
                          ? 3.0
                          : 0.0), // Add more padding if image is above
                  child: Text(
        ...
        return Container(
          ...
          child: ConstrainedBox(
            constraints: const BoxConstraints(
                minHeight: 35.0), // Adjust height as needed
            child: content,
          ),
        );
      },
  ...
  Widget build(BuildContext context) {
    super.build(
        context); // Ensure super.build is called for AutomaticKeepAliveClientMixin
    ...
    return Container(
      margin: const EdgeInsets.symmetric(
          vertical: 4.0), // Default gap handled by separatorBuilder now
      child: Column(
...
class _DateSeparator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Container(
        margin:
            const EdgeInsets.symmetric(vertical: 12.0), // Add vertical margin
